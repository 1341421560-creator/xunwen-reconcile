param([string]$Archive = '', [string]$CompanyKey = '', [switch]$AllCompanies)
...
$arguments = @('-B','-X','utf8',(Join-Path $PSScriptRoot 'entry.py'),'data','restore','--archive',(Resolve-Path -LiteralPath $Archive).Path)
