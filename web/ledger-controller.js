...
export function createController(state){
 ...
 function apply(result){if(state.result&&result.company.id===state.result.company.id&&result.revision<state.result.revision)return;state.result=result;setReady(true);if(state.month&&!result.months.includes(state.month))state.month='';renderLedger(state);renderConflicts(state,mutate);expenseStats.onLedger();}
 ...
 function bind(){
  setReady(false);expenseStats.bind();
  document.addEventListener('click',async e=>{const target=e.target.closest('button');if(!target)return;try{
   ...
   if(target.dataset.bankNote)openBankNote(state.result,target.dataset.bankNote,mutate);
   if(target.dataset.expenseCategory)openExpenseCategory(state.result,target.dataset.expenseCategory,mutate);
  ...
  el('settings-form').onsubmit=async e=>{e.preventDefault();try{const aliases={};for(const line of el('aliases').value.split('\n').map(s=>s.trim()).filter(Boolean)){const parts=line.split('=');if(parts.length!==2||!parts.every(s=>s.trim()))throw new Error('名称映射每行使用：银行对方名称 = 发票销方名称');const [k,v]=parts.map(s=>s.trim());if(k in aliases&&aliases[k]!==v)throw new Error('同一银行名称不能对应多个销方');aliases[k]=v;}await mutate('/api/settings',{revision:state.result.revision,aliases,exclude_special:el('exclude-special').checked,custom_exclude_keywords:readSummaryKeywords(el('summary-keywords-editor'))},false);}catch(error){message(error.message,true);}};
