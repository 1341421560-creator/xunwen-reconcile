...
import {renderInvoiceTable} from './invoice-table.js';
import {paginate,renderPager as pager} from './pagination.js';
...
export function scopedBanks(state){return state.result.bank.filter(b=>(state.unfinished?['unmatched','partial','review'].includes(b.status):!state.month||b.date.startsWith(state.month)));}
export function renderTables(state){
 ...
 renderInvoiceTable(state,badge);
}
export function renderHistory(state){
 el('batch-list').innerHTML=[...state.result.batches].reverse().map(b=>`<article class="history-item"><div><strong>${esc(b.at)}${b.legacy_session_id?' · 旧记录迁入':''}</strong><p>${b.files.map(f=>esc(f.name)).join('<br>')}</p><p>${['bank','invoices'].map(k=>`${k==='bank'?'流水':'发票'}：新增 ${b.counts[k].new}，重复 ${b.counts[k].duplicate}，冲突 ${b.counts[k].conflict}`).join('；')}</p><p>本次自动关联 ${b.auto_matched_count} 笔，其中历史付款 ${b.historical_matched_count} 笔</p><details class="source-details"><summary>批次来源与导入检查</summary><p class="record-id">${esc(b.id)}</p>${b.files.map(f=>`<p>${esc(f.name)} · ${f.rows} 行<br><span class="record-id">SHA-256 ${esc(f.sha256)}</span></p>`).join('')}${b.notes.map(n=>`<p>${esc(n)}</p>`).join('')}${b.controls.map(c=>`<p>${esc(c.sheet)} · ${esc(c.label||c.type)} 控制数 ${money(c.cents)} / 实际 ${money(c.actual_cents)} ${Number.isInteger(c.expected_count)?`· 笔数 ${esc(c.actual_count)} / ${esc(c.expected_count)}`:''} · ${c.passed?'通过':'不一致'}</p>`).join('')}</details></div></article>`).join('')||'<div class="empty">还没有导入记录</div>';
 el('legacy-list').innerHTML=state.history.map(h=>`<article class="history-item"><div><strong>${esc(h.saved_at)}</strong><p>${esc(h.bank_name)}<br>${esc(h.invoice_name)}</p></div><button class="small-button" data-legacy="${esc(h.id)}">只读查看</button></article>`).join('')||'<p class="muted">无旧版记录</p>';
 el('audit-list').innerHTML=[...state.result.audit].reverse().slice(0,100).map(a=>`<div class="audit-entry">${esc(a.at)} · 版本 ${a.revision} · ${esc(a.action)}<br>${esc(a.note)}<span class="record-id"> ${esc((a.allocation_ids||[]).join(' / '))}</span></div>`).join('');
}
export function renderLedger(state){
 const r=state.result;el('difference-filter').innerHTML='<option value="all">全部差额状态</option>'+Object.entries(r.difference_labels).map(([key,label])=>`<option value="${esc(key)}">${esc(label)}</option>`).join('');el('difference-filter').value=state.differenceFilter;el('company').textContent=r.company.name;el('ledger-meta').textContent=`累计账本 · 版本 ${r.revision} · ${r.saved_at?'已保存 '+r.saved_at:'尚未导入'}`;
