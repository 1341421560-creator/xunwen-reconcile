...
def export_ledger(view, root, labels, month=""):
    ...
    write_csv(directory / "流水对账结果.csv", ["流水编号", "付款日期", "对方户名", "摘要", "方向", "币种", "支出金额", "收入金额", "累计有效核销金额", "剩余金额", "状态", "依据", "银行账号", "银行流水号", "原文件", "原工作表", "原行号", "导出时间"], [
        [b["id"], b["date"], b["party"], b["summary"], b["direction"], b["currency"], amount(b["debit_cents"]), amount(b["credit_cents"]), amount(b["allocated_cents"]), amount(b["remaining_cents"]), labels[b["status"]], b["reason"], b.get("account", ""), b.get("reference", ""), b.get("source", ""), b.get("sheet", ""), b.get("row", ""), at] for b in banks])
    write_csv(directory / "发票余额明细.csv", ["发票编号", "发票代码", "发票号码", "实际开票日期", "销方", "票面价税合计", "全账本已分配金额", "全账本可用余额", "状态", "源状态", "导出时间"], [
        [i["id"], i.get("code", ""), "'" + i["number"], i["date"], i["party"], amount(i["amount_cents"]), amount(i["allocated_cents"]), amount(i["remaining_cents"]), labels[i["status"]], i.get("source_status", ""), at] for i in invoices])
