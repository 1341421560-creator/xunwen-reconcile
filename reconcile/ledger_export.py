...
from .invoice_offset_export import offset_export_scope, export_offsets
...
def export_ledger(view, root, labels, month=""):
    ...
    iids, offsets = offset_export_scope(view, iids, not month)
    invoices = [i for i in view["invoices"] if not month or i["id"] in iids]
    bm, im = {b["id"]: b for b in banks}, {i["id"]: i for i in invoices}
    export_offsets(directory, offsets, im, at)
    write_csv(directory / "流水对账结果.csv", ["流水编号", "付款日期", "对方户名", "摘要", "方向", "币种", "支出金额", "收入金额", "累计有效核销金额", "剩余金额", "状态", "依据", "银行账号", "银行流水号", "原文件", "原工作表", "原行号", "关联发票差额提示", "手工备注", "备注更新时间", "支出类别", "支出分类依据", "相关发票提示", "关联发票冲红提示", "导出时间"], [
        [b["id"], b["date"], b["party"], b["summary"], b["direction"], b["currency"], amount(b["debit_cents"]), amount(b["credit_cents"]), amount(b["allocated_cents"]), amount(b["remaining_cents"]), labels[b["status"]], b["reason"], b.get("account", ""), b.get("reference", ""), b.get("source", ""), b.get("sheet", ""), b.get("row", ""), b["difference_hint"], b.get("manual_note", ""), b.get("manual_note_updated_at", ""), next((item["label"] for item in view["expense_categories"] if item["id"] == b["expense_category"]), ""), b["expense_category_reason"], b.get("related_invoice_hint", ""), b.get("invoice_offset_hint", ""), at] for b in banks])
    write_csv(directory / "发票余额明细.csv", ["发票编号", "发票代码", "发票号码", "实际开票日期", "销方", "票面价税合计", "全账本已分配金额", "全账本未分配余额", "状态", "源状态", "可继续分配金额", "差额状态", "差额处理依据", "差额更新时间", "累计冲红金额", "冲红后可核销总额", "对应票号", "冲红处理状态", "导出时间"], [
        [i["id"], i.get("code", ""), "'" + i["number"], i["date"], i["party"], amount(i["amount_cents"]), amount(i["allocated_cents"]), amount(i["remaining_cents"]), view["offset_labels"][i["offset_status"]] if i["red"] or i["status"] == "offset_full" else labels[i["status"]], i.get("source_status", ""), amount(i["distributable_cents"]), view["difference_labels"][i["difference_status"]], i["difference_note"], i["difference_updated_at"] or "", amount(i["offset_cents"]), amount(i["net_amount_cents"]), "、".join(i["offset_invoice_numbers"]), view["offset_labels"][i["offset_status"]], at] for i in invoices])
    ...
    snapshot = dict(view, bank=banks, invoices=invoices, allocations=allocations, stats=stats, filtered_stats=stats,
                    ...
                    conflicts=conflicts, audit=audit, invoice_offsets=offsets, batches=[b for b in view["batches"] if not month or b["id"] in batch_ids],
